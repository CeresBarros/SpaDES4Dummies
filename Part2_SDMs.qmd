...
In [Chapter 1](Part1_DummyModel.qmd) of this guide, we described how to create new `SpaDES` modules, their different components, how to link different modules and how to set up and run a SpaDES worflow.
...
With the above in mind, in this example we created three modules that source and pre-process data ('data modules') and a module that fits an SDM and iteratively forecasts species distributions (we call it a 'prediction module').
The idea is that we could, for instance, provide different data sources to one of the data modules and only update the parts of the workflow that are affected by this change (i.e. presumably the other data module steps with not be affected).
Or, we could develop a second prediction module using a different SDM approach and swap the two modules to inspect which provides better predictions.

Our data modules are *speciesAbundanceData* and *climateData*.
The prediction module is *projectSpeciesDist*.
We start by creating an `.R` script to set up and control the worflow.
...
Finally, note that we do not load any R packages yet, as we will later use `Require` to make sure all module dependencies are installed before running the worflow (see [Reproducible package installation]).
...
The `data.table` contains added information about the year of the worflow during which the data should be used (here, only the first year when SDM fitting happens).
...
It relies on two input `data.tables` that contain the URLs for each climate covariate, one for baseline conditions, the other for projected climate conditions, both containing information about when each layer should be used during the worflow (the "year"column).
...
If providing other URLs to obtain different climate data, pay special attention to the "year" column of `projClimateURLs` -- the URLs need to correspond to the year during which they will be used (not necessarily the actual climate year, unless the years follow the same numbering).
...
-   How, unlike `fitSDM`, the `projSDM` event schedules itself so that model projections are executed for each year, provided that there is corresponding environmental data -- notice how the functions `fitSDMEvent` and `projSDMEvent` both check that there is data for the current year of the workflow run (`time(sim)`).

-   How the fitted model object (`sdmOut`) and it's evaluation (`evalOut`) are both module outputs.
    This way these objects can not only be used by other events, but also inspected by the user after the worflow run is finished (see [Transparent models]).
...
If you can't install Java (e.g. no admin rights) you will still be able to run the using the generalised linear model (GLM) option.

### Workflow set-up

After the modules are created, we go back to the `Part2_SDMs.R` script to set up and run the simulation.

We will use `SpaDES.project::setupProject` to setup the project folder tree and the the simulation.

Briefly, `setupProject` will create a full project folder structure (it will add folders to `projPath` like `inputs/` and `outputs/`) and an RStudio Project there.
It will also look at all package dependencies listed in the modules and install them, amongst many other things (see e.g. `sideEffects`).
Basically, `setupProject` creates a list of arguments that `simInit` needs to initialise the workflow/simulation, such as:
...
Notice the use of `paths` (an internal object that will become a slot in `mySimMaxEnt`) to refer to the cache (`cachePath`) and inputs (`inputPath`) folders created by `setupProject`.

The `studyAreaRas` is internally created from a random polygon drawn in SW Alberta, Canada, using `SpaDES.tools::randomStudyArea` (@fig-studyAreaCanada), by executing the code chunk passed to `studyAreaRas` (an argument passed to `setupProject(...)`).
...
If you face issues installing packages, this [troubleshooting guide](https://predictiveecology.org/training/_book/TroubleshootingPackageInstallation.html) can help.
If yousing RStudio, pay special attention to ["Package has already been loaded"](https://predictiveecology.org/training/_book/TroubleshootingPackageInstallation.html#package-has-already-been-loaded) section.
:::
...
Now we can create a very similar list for the workflow using the GLM modelling approach.
All we do is create a copy of `mySimMaxEnt` and change a parameter value:
...
SpaDES workflows use the [`simList`](https://spades-core.predictiveecology.org/reference/simList-class.html) object with can only be created using `SpaDES.core::simInit()`.
`simInit()` also runs all the `.inputObjects()` functions of the modules being used to prepare default inputs supplied by the developer.
...
-   `simInit()` and `simInit2()` are identical in that they create the `simList` object, but `simInit2()` accepts a `list` of named arguments, such as the output of `setupProject()`[^part2_sdms-1].
...
In this example, we relied on caching to avoid repeating computationally intensive or long operations.
...
In the example above, we rely on `setupProject` to install all module package dependencies and a few extra ones[^part2_sdms-2]

If not using `setupProject`[^part2_sdms-3] `options("spades.useRequire")` is set to `TRUE` (the default), `SpaDES` will automatically attempt to install any packages listed across all modules if they are not installed in `.libPaths()`, or if the installed version (or branch if installing from GitHub) does not correspond to what is listed in the module `.R` script.
...
Please beware that package installation should be done as much as possible from a clean R session especially in the context of a `SpaDES`-based project, where each module can potentially have many different dependencies, which have dependencies of their own (notice how we didn't load any packages other than `SpaDES.project`).[^part2_sdms-4]
...
[^part2_sdms-1]: See Chapter 7 - The simList - What is it?
    of our book \[*Robust and nimble scientific workflows, using SpaDES*\](<https://predictiveecology.org/training/_book/simList.html>)for a use case of `simInit()` and more details and examples on the `simList` object.

[^part2_sdms-2]: Via the `setupProject(..., packages)` argument.

[^part2_sdms-3]: I.e., if creating the required inputs for `simInit` "manually" on the global environment and passing them to `simInit()`.
    We, SpaDES developers, are gradually moving away from this practice to keep the global environment as "clean" as possible [Robust and nimble scientific workflows, using SpaDES - Best practices](#0){style="font-size: 13pt;"}.

[^part2_sdms-4]: See also [Robust and nimble scientific workflows, using SpaDES - 3  Troubleshooting R package installation](https://predictiveecology.org/training/_book/TroubleshootingPackageInstallation.html).
