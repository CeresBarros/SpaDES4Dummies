...
```{r globalscriptPart2, eval = FALSE, message = FALSE, warning = FALSE, results = 'hide', class.source = "fold-show"}
...
```{r globalscriptPart2Hidden, include = FALSE, cache = FALSE}
...
```{r plotSpatRaster, code = readLines(file.path("modules", "projectSpeciesDist", "R", "plotSpatRaster.R")), eval = FALSE}
```

The `climateData` module uses the function `plotSparRasterStk`, which is
in the `plotSparRasterStk.R` in its `R/` folder:

```{r plotSpatRasterStk, code = readLines(file.path("modules", "climateData", "R", "plotSpatRasterStk.R")), eval = FALSE}
```
...
```{r simulationPkgs, message = FALSE, warning = FALSE, results = 'hide'}
...
```{r moduleDiagram2, echo = FALSE, fig.align = 'center', fig.cap = "Module network diagram.", message=FALSE, warning=FALSE, fig.height=6, fig.width=6}
moduleDiagram(mySimMaxEnt)
```

```{r objectDiagram2, echo = FALSE, fig.align = 'center', fig.cap = "Module diagram showing module inter-dependencies with object names."}
