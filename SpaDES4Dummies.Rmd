...
Now the rest of the script determines the event sequence - remember SpaDES = Spatial Descrete Event Simulator - and defines the events themselves. The initialisation (`init`) event is **mandatory**, the others are not. SpaDES knows how to deal with an `init` event and "searches" for it. For example, it knows that the initialisations of all active modules will be executed first. The order in which modules (and consequently their initialisations) are executed is automatically determined by inter-module dependencies (i.e. if modules require inputs that are the outputs of other modules) - I told you SpaDES is cool! If there are no inter-module dependencies the order is determined by the other in which modules are listed in the *global.R* script.
...
I encourage you to check what `base::switch` is doing on your own, but if you're lazy here's a dummy explanation: it tells R that the behaviour of your function will change (or switch) depending on what the `eventType`. So we need to define the behaviour of `doEvent` for all the event types we have in this module. In each of them we will execute the event functions and schedule future events with `scheduleEvent`.

The first one is, obviously, `init` - don´t change the name of this one. We run the initialisation event and schedule the abundance simulation and plotting events. Notice that the `abundanceSim` event will be generated at `start(sim)` (i.e. at the starting time), which basically is the same as running it during the initialisation. The first plotting event will be executed at the time defined by the `.plotInitialTime` parameter, which is store in the `sim` object (and obtained using  `SpaDES.core::P`). 

The plot event type is defined next and besides doing the obvious plotting, it **schedules itself** to occur at a frequency defined by the `.plotInterval` parameter - isn't that cool? I think it is, but I'm only a silly ecologist.

Finally, the `SimulAbund` event is very similar to the plotting one, the only real difference being that it uses the `simulationTimeStep` time step to re-schedule itself.
Note: Notice how I've scheduled future events to `time(sim) + P(sim)$simulationTimeStep` or `time(sim) + P(sim)$.plotInterval`.

Ok, just one more bit left. The actual species abundances generator function, which I have coded at the end of my *speciesAbundance.R* script:
...
```
Did you see how the future events where scheduled to `P(sim)$statsTimestep + 0.1`? This forces teh statistic analyses to occur **after** the abundance and temperature rasters are ready.


### Let's play!
Ok, now that we have our modules ready we can set up the simulation. Let's go back to our *global.R* script. 
The function `simInit` will take a bunch of parameter lists and will prepare a simualtion object that can later be run by the `spades` function. Let's see what these lists consist of:
...
```
To make sure that plotting always occurs after the abundances are generated, we'll affect this parameter with 1.5.
