...
install.packages("SpaDES.project", "SpaDES.core")
...
## now, assuming you've created your modules and each sources a series of other packages
...
mySimMaxEnt <- setupProject(paths = list("packagePath" = "packages/",
                            ...
                            packages = c("reproducible",
                                         # "SpaDES.core", ## not needed it's in the module already
                                         ## the following packages are needed outside the modules
                                         "SpaDES.experiment",
                                         "ggpubr",
                                         "geodata",
                                         "SpaDES.tools"#,
                                         # "PredictiveEcology/SpaDES.experiment@75d917b70b892802fed0bbdb2a5e9f3c6772f0ba" -- we already have SpaDES.experiment above and will get it from "https://predictiveecology.r-universe.dev/"
                                         ),
                            params = list(
...
if (!require(rJava, quietly = TRUE)) {
  ...
  message("If you need to install Java without admin priviledges, see:",
          "\n  https://stackoverflow.com/a/13355764/11969696")
}
...
mySimGLM2 <- mySimGLM
projClimateURLsNew <- copy(simInitMaxEnt2$projClimateURLs)
projClimateURLsNew[, `:=`(URL = gsub("ssp585", "ssp126", URL),
                       targetFile = gsub("ssp585", "ssp126", targetFile))]

## we simply replace the projClimateURLs and run simInit2() again
## to change the climate layers
...
myExperiment2 <- experiment2(MaxEnt = simInitMaxEnt2, 
                             GLM = simInitGLM2, 
                             debug = TRUE, 
                             replicates = 1,
                             clearSimEnv = FALSE)
