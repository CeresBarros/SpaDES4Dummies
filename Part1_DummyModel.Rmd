...
```{r loadPkgs, eval = FALSE, message = FALSE, warning = FALSE, results = 'hide', class.source = "fold-show"}
...
```{r loadPkgsHidden, include = FALSE}
library(terra)
library(quickPlot)
library(ggplot2)
library(ggpubr)
...
```{r the_r_way_raster, message = FALSE, warning = FALSE, class.source = "fold-show"}
...
```{r the_r_way_simulation_function_AB, class.source = "fold-show"}
...
```{r the_r_way_simulation_length, message=FALSE, class.source = "fold-show"}
Time <- 10
...
```{r the_r_way_simulation_function_TMP, class.source = "fold-show"}
...
```{r the_r_way_plot_results_TMP, class.source = "fold-show"}
...
```{r data_analysis_model, class.source = "fold-show"}
...
```{r data_analysis, class.source = "fold-show"}
lmPlots <- list()
for (t in 1:Time) {
...
```{r globalscriptPart1, eval = FALSE, message = FALSE, warning = FALSE, results = 'hide'}
...
```{r globalscriptPart1Hidden, include = FALSE, cache = FALSE}
...
```{r figObj, fig.align = 'center', warning = FALSE, message = FALSE, echo = FALSE, fig.cap = "Inputs and outputs in SpaDES: Object A comes from outside of the module (e.g. from an internet URL, from data you have, or from `.inputObjects`), while Module Z produces object C. Both objects serve as an inputs for Module Y, which in return produce as outputs objects B and D, respectivelly from objects A and C. As Module Z uses a simple function *internally* to create object C, it doesn't have any inputs, such as our dummy example.", out.width="80%"}
...
```{r abund_module8, code = readLines(file.path(projDir, "modules", "speciesAbundance", "R", "abundance_model.R")), eval = FALSE}
```
...
```{r temp_module, code = readLines(file.path(projDir, "modules", "temperature", "temperature.R")), eval = FALSE}
```

Again, we added an accessory `temperature_model` function in a separate script `R/temperature_model.R`:

```{r temp_module2, code = readLines(file.path(projDir, "modules", "temperature", "R", "temperature_model.R")), eval = FALSE}
```
...
```{r stats_module, code = readLines(file.path(projDir, "modules", "speciesTempLM", "speciesTempLM.R")), eval = FALSE}
```

And the script with the accessory functions:

```{r stats_module2, code = readLines(file.path(projDir, "modules", "speciesTempLM", "R",  "linear_model_functions.R")), eval = FALSE}
```
...
```{r moduleDiagram, fig.height=6, fig.width=6, message=FALSE, warning=FALSE, fig.cap="Diagram of module connections."}
moduleDiagram(mySim)
```
...
```{r objectDiagramCode, eval=FALSE}
objectDiagram(mySim)
```

```{r objectDiagram, echo = FALSE, fig.align = 'center', fig.cap = "Module diagram showing module inter-dependencies with object names.", fig.width = 10}
...
```{r figSimulationFig, fig.align = 'center', warning = FALSE, message = FALSE, echo = FALSE, fig.cap = "Simulation plots: Final plot of the simulation", out.width="80%"}
