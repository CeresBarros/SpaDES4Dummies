...
#    `functionName` is a function's name in your module (e.g., `temperatureEvent1`).
# 2. Copy this file to the tests folder (i.e., `D:/GitHub/SpaDES4Dummies/modules/temperature/tests/testthat`).

# 3. Modify the test description based on the content you are testing:
test_that("test Event1 and Event2.", {
  module <- list("temperature")
  path <- list(modulePath = "D:/GitHub/SpaDES4Dummies/modules",
               outputPath = file.path(tempdir(), "outputs"))
  parameters <- list(
    #.progress = list(type = "graphical", interval = 1),
    .globals = list(verbose = FALSE),
    temperature = list(.saveInitialTime = NA)
  )
  ...
  if (exists("temperatureEvent1", envir = .GlobalEnv)) {
    simOutput <- temperatureEvent1(mySim)
  } else {
  ...
  if (exists("temperatureEvent2", envir = .GlobalEnv)) {
    simOutput <- temperatureEvent2(mySim)
  } else {
